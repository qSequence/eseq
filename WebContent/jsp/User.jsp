...
<tr><td><input type="hidden" id="team_id_param" name="team_id_param" value="${modelUc.editedUser.teamId}"/>
<form id="subscription_order_form" action="User.htm" method="post"  >
	
	<table align="center" border="0" width=600>
			<tr><td colspan="2">&nbsp;</td></tr>
			<tr><td colspan="2">&nbsp;</td></tr>
		<tr><td style="text-align:left;"><h1>Users</h1></td><td style="text-align:right;">&nbsp;<input type="hidden" name="user_request_action_param" value="save_action"/></td><td>&nbsp;</td><td>&nbsp;</td></tr>
		
        <tr style="height:50px;text-align:left;">
        	<td ><label>First name </label></td><td> <input type="text" name="first_name_param" value="${modelUc.editedUser.firstName}" maxlength="100"/><input type="hidden" name="user_id_param" value="${modelUc.editedUser.id}" /></td>
        	<td><label>Last name </label></td><td> <input type="text" name="last_name_param" value="${modelUc.editedUser.lastName}" maxlength="100"/><input type="submit" value="Save" />
		</td></tr>
		<tr style="height:50px;text-align:left;">
			<td ><label>Email Address </label> </td><td><input type="text" name="email_address_param" value="${modelUc.editedUser.userNameEmail}" maxlength="100"/></td>
			<td><label>Phone </label> </td><td><input type="text" name="phone_param" value="${modelUc.editedUser.phone}" maxlength="30" />
		</td></tr>
		<tr style="height:50px;text-align:left;">
			<td ><label>Username </label></td><td> <input type="text" name="username_param" value="${modelUc.editedUser.userName}" maxlength="100"/></td>
			<td><label>User type</label></td><td> <select name="user_type_param" id="user_type_param" >
		<option value=""></option>
		...
		<tr style="height:50px;text-align:left;"><td><label>Set Password</label></td><td colspan="2"> <input type="password" name="password_param" maxlength="100" /></td>
	   <td><label>Source</label></td><td> <select name="source_param" id="source_param" >
		<option value=""></option>
		<option value="LDAP" selected>Ldap</option>
		</select> 
	   </td>
	   </tr>
        <tr style="height:50px;text-align:left;"><td colspan="4" >
        	<div id="div_id" style="overflow:auto;height:300px;width:700px;border: 2px solid grey;">
				<table ><tr><td><label><strong>Add Teams</strong></label></td></tr>
                        <tr style="height:50px;text-align:left;"><td><label>Team</label><input type="text" id="team_name_param" name="team_name_param"  maxlength="100" value="${modelUc.editedUser.userTeamName}"/><label>Team type</label> <select name="access_param" id="access_param" ><option value="PT">Permanent team</option><option value="TT">Temporary team</option></select><button type="button" onclick="addLi(document.getElementById('team_name_param').value, document.getElementById('team_id_param').value, document.getElementById('access_param').value)">Add</button></td></tr>
...
<c:set var="x" value="0" />
			...
			<c:if test="${modelUc.userList[0]!=null}">
        		...
        		<tr style="background-color:#E8E8E8; color: black;">
					...
					<td align="center">
						<a href="<c:url value="User.htm?user_request_action_param=delete_action&user_id_param=${user.id}"/>" onclick="return confirm('Do you really want to DELETE?')"><img id='${user.id}'  src="images/delete.png"/></a></td>
					</td>
					<c:set var="x" value="0" />
